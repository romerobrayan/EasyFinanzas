...
## Current sprint — Sprint 3: automatic capture, SMS first (done)

Brief: `TASK_SPRINT_3_CAPTURE.md`. Delivered: bank SMS (Bancolombia `85540` + the 1CERO1 work card `891134`) captured via `core/data/capture/SmsCaptureReceiver` (live) and a 90-day `READ_SMS` backfill (`SmsCaptureSource`), both gated on the explicit opt-in in `feature/capture/CaptureSetupScreen`. Parsing is `core/data/capture/parser/RuleBasedTransactionParser` over data rule sets (`TintoIssuerRules`): per-issuer decimal conventions with integer-math amount parsing (`AmountParser`), per-issuer date layouts against America/Bogotá (`DateLayouts`), direction, last-4 card match, drop patterns. Every parse stages into a device-local Room store (`core/data/local`, `pending_transactions` + a `capture_seen` dedupe index) behind `PendingTransactionRepository` — **never auto-committed**. Review flows through the dashboard "Detectamos N movimientos" banner → `feature/pending/PendingReviewScreen` (new `PendingRoute`): editable sheet, category required, `DetectPendingDuplicates` flags same-amount ±10 min pairs (mask NOT required to match — cross-issuer pairs are the point); confirm promotes through the session-routed `TransactionRepository` with `source = SMS` preserved. Raw SMS bodies stay on-device; analytics carry channel + issuer only. `NotificationListenerService` (Nu) and Gmail remain stubbed seams (`// TODO(sprint-4):`).
